<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="公告文件" prop="gonggaowenjian">
          <file-upload
          tip="点击上传公告文件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.gonggaowenjian" label="公告文件" prop="gonggaowenjian">
            <el-button type="text" size="small" @click="download(ruleForm.gonggaowenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="24">  
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yiyuangonggao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yiyuangonggaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
