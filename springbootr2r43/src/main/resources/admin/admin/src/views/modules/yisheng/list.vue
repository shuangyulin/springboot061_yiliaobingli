<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yisheng','加入医生展示') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yishengzhanshiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入医生展示':'' }}</el-button>
                <el-button v-if="isAuth('yisheng','加入医生展示') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yishengzhanshiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入医生展示':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yisheng','加入医生展示') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yishengzhanshiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'加入医生展示':'' }}</el-button>
    ...
    <yishengzhanshi-cross-add-or-update v-if="yishengzhanshiCrossAddOrUpdateFlag" :parent="this" ref="yishengzhanshiCrossaddOrUpdate"></yishengzhanshi-cross-add-or-update>
...
import yishengzhanshiCrossAddOrUpdate from "../yishengzhanshi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yishengzhanshiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    yishengzhanshiCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    yishengzhanshiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yishengzhanshiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yisheng');
      this.$nextTick(() => {
      this.$refs.yishengzhanshiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          zhuanyetezhang: row.zhuanyetezhang,
