<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="发表时间" prop="fabiaoshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.fabiaoshijian" 
                type="datetime"
                placeholder="发表时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fabiaoshijian" label="发表时间" prop="fabiaoshijian">
              <el-input v-model="ruleForm.fabiaoshijian" 
                placeholder="发表时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shipin = true;
          ...
          if(o=='fabiaoshijian'){
            this.ruleForm.fabiaoshijian = obj[o];
	    this.ro.fabiaoshijian = true;
            continue;
          }
          if(o=='wenzhangtupian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.yiyuanbianhao!=''&&json.yiyuanbianhao){
              		this.ruleForm.yiyuanbianhao = json.yiyuanbianhao
		}
        } else {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springbootr2r43/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yiyuanwenzhang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yiyuanwenzhangCrossAddOrUpdateFlag = false;
                  this.parent.search();
